 ...
 var editproductname;
    ...
    function loadProducts() {  
        ...
        var xhr = Ti.Network.createHTTPClient();
    	  xhr.onload = function(e) {
             products = eval('('+this.responseText+')');
               ...
               for (var i = 0; i < products.length; i++) {
               	...
               	productidarray[i]= products[i].id; 
					}             
               for (var i = 0; i < products.length; i++) {  
           		     var  name  = products[i].name; // The tweet message
    	 ...
    	 xhr.open('GET','http://192.168.0.16:3000/productproducts.json'); 
		 xhr.send(); 
        ...
        function addeventforeditanddelete(){
        				 for (var i = 0; i < deletebarray.length; i++) {
               				...
               				deletebarray[i].addEventListener('click',function(e)
								{
								   var xhr = Ti.Network.createHTTPClient();
    	  								 xhr.open('DELETE','http://192.168.0.16:3000/productproducts/'+e.source.titleid); 
		 						       xhr.send(); 		 
